...
export function validateProfile(profile, { registry = [], gpuCap } = {}) {
  ...
  const cap = Number.isInteger(profile.gpu)
    ? (gpuCap && Number.isFinite(gpuCap[profile.gpu]) ? gpuCap[profile.gpu] : GPU_CAP_GB[profile.gpu])
    : undefined;
  if (ctxTokens > 0 && cap) {
    const kvGb = (ctxTokens * kvBytesPerToken) / 1e9;
    if (kvGb > cap - 1) {
      warnings.push(
        `KV 估算 ~${kvGb.toFixed(0)}G,超出 GPU ${profile.gpu} 的 ${cap}G 容量(KV 单独估算,` +
